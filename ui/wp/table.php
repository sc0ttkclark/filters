...
class Filters_Posts_List_Table extends WP_Posts_List_Table {
	...
	public function __construct ( $_post_type = null, $post_type_object = null ) {
        ...
        if ( !current_user_can( self::$filters_post_type_object->cap->edit_others_posts ) ) {
            $this->user_posts_count = $wpdb->get_var( $wpdb->prepare( "
				...
				AND post_author = %d
			", self::$filters_post_type, get_current_user_id() ) );

            if ( $this->user_posts_count && empty( $_REQUEST[ 'post_status' ] ) && empty( $_REQUEST[ 'all_posts' ] ) && empty( $_REQUEST[ 'author' ] ) && empty( $_REQUEST[ 'show_sticky' ] ) )
                $_GET[ 'author' ] = get_current_user_id();
        }

        if ( 'post' == self::$filters_post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
            $sticky_posts = implode( ', ', array_map( 'absint', (array) $sticky_posts ) );
            $this->sticky_posts_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( 1 ) FROM $wpdb->posts WHERE post_type = %s AND post_status != 'trash' AND ID IN ($sticky_posts)", self::$filters_post_type ) );
        }
    ...
    public function get_views() {
        ...
        $num_posts = wp_count_posts( self::$filters_post_type, 'readable' );
        $class = '';
        $allposts = '';
        ...
        if ( $this->user_posts_count ) {
            ...
            $url = filters_var_update( array( 'post_type' => self::$filters_post_type, 'author' => $current_user_id, 'paged' => '', 'action' => '', 'action2' => '' ), null, array( 'paged', 'action', 'action2' ) );
        ...
        $url = filters_var_update( array( 'post_type' => self::$filters_post_type, 'post_status' => '', 'all_posts' => ( empty( $allposts ) ? '' : 1 ), 'paged' => '', 'action' => '', 'action2' => '' ), null, array( 'paged', 'action', 'action2' ) );
        ...
        foreach ( get_post_stati( array( 'show_in_admin_status_list' => true ), 'objects' ) as $status ) {
            ...
            $url = filters_var_update( array( 'post_type' => self::$filters_post_type, 'post_status' => $status_name, 'paged' => '', 'action' => '', 'action2' => '' ), null, array( 'paged', 'action', 'action2' ) );
        ...
        if ( !empty( $this->sticky_posts_count ) ) {
            ...
            $url = filters_var_update( array( 'post_type' => self::$filters_post_type, 'show_sticky' => 1, 'paged' => '', 'action' => '', 'action2' => '' ), null, array( 'paged', 'action', 'action2' ) );
...
?>
    <div class="alignleft actions">
        <?php
            if ( $this->is_trash && current_user_can( self::$filters_post_type_object->cap->edit_others_posts ) ) {
...
?>
    <div id="filters-ui-posts-filter-popup" class="hidden">
        <form action="" method="get" class="filters-ui-posts-filter-popup">
            ...
            <p class="submit<?php echo ( $zebra ? ' clear' : '' ); ?>"><input type="submit" value="<?php esc_attr_e( 'Search', 'filters' ); ?> <?php echo self::$filters_post_type_object->label; ?>" class="button button-primary" /></p>
        </form>
    </div>
