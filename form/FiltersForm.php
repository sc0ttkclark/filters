...
class FiltersForm {
    ...
    public static function label ( $name, $label, $help = '', $options = null ) {
        ...
        $label = apply_filters( 'filters_form_ui_label_text', $label, $name, $help, $options );
        $help = apply_filters( 'filters_form_ui_label_help', $help, $name, $label, $options );
        ...
        $type = 'label';
        $attributes = array();
        $attributes[ 'class' ] = 'filters-form-ui-' . $type . ' filters-form-ui-' . $type . '-' . $name_more_clean;
        $attributes[ 'for' ] = ( false === strpos( $name_clean, 'filters-form-ui-' ) ? 'filters-form-ui-' : '' ) . $name_clean;
        $attributes = self::merge_attributes( $attributes, $name, $type, $options, false );

        filters_view( FILTERS_DIR . 'ui/fields/_label.php', compact( array_keys( get_defined_vars() ) ) );

        $output = ob_get_clean();

        return apply_filters( 'filters_form_ui_' . $type, $output, $name, $label, $help, $attributes, $options );
    }
    ...
    public static function field ( $name, $value, $type = 'text', $options = null, $pod = null, $id = null ) {
        ...
        if ( false === FiltersForm::permission( $type, $name, $options, null, $pod, $id ) )
            return false;

        $value = apply_filters( 'filters_form_ui_field_' . $type . '_value', $value, $name, $options, $pod, $id );

        ob_start();

        $helper = false;

        if ( 0 < strlen( filters_var_raw( 'input_helper', $options ) ) )
            $helper = filters_api()->load_helper( array( 'name' => $options[ 'input_helper' ] ) );
        ...
        if ( !empty( $helper ) && 0 < strlen( filters_var_raw( 'code', $helper ) ) && ( !defined( 'FILTERS_DISABLE_EVAL' ) || !FILTERS_DISABLE_EVAL ) )
            eval( '?>' . $helper[ 'code' ] );
        ...
        elseif ( is_object( self::$loaded[ $type ] ) && method_exists( self::$loaded[ $type ], 'input' ) )
            ...
            do_action( 'filters_form_ui_field_' . $type, $name, $value, $options, $pod, $id );

        $output = ob_get_clean();

        return apply_filters( 'filters_form_ui_field_' . $type, $output, $name, $value, $options, $pod, $id );
    }
    ...
    protected static function field_hidden ( $name, $value = null, $options = null ) {
        ...
        return apply_filters( 'filters_form_ui_field_hidden', $output, $name, $value, $options );
    }
    ...
    public static function attributes ( $attributes, $name = null, $type = null, $options = null ) {
        $attributes = (array) apply_filters( 'filters_form_ui_field_' . $type . '_attributes', $attributes, $name, $options );
    ...
    public static function data ( $data, $name = null, $type = null, $options = null ) {
        $data = (array) apply_filters( 'filters_form_ui_field_' . $type . '_data', $data, $name, $options );
    ...
    public static function merge_attributes ( $attributes, $name = null, $type = null, $options = null, $classes = '' ) {
        ...
        if ( !in_array( $type, array( 'label', 'comment' ) ) ) {
            ...
            if ( 0 < strlen( filters_var_raw( 'label', $options, '' ) ) )
                $_attributes[ 'data-label' ] = strip_tags( filters_var_raw( 'label', $options ) );

            $_attributes[ 'id' ] = 'filters-form-ui-' . $name_clean;
            $_attributes[ 'class' ] = 'filters-form-ui-field-type-' . $type . ' filters-form-ui-field-name-' . $name_more_clean;

            if ( isset( $options[ 'dependency' ] ) && false !== $options[ 'dependency' ] )
                $_attributes[ 'class' ] .= ' filters-dependent-toggle';
        ...
        if ( 1 == filters_var( 'required', $options, 0 ) )
            $attributes[ 'class' ] .= ' filters-validate filters-validate-required';
        ...
        $attributes = (array) apply_filters( 'filters_form_ui_field_' . $type . '_merge_attributes', $attributes, $name, $options );
        return $attributes;
    }
    ...
    public static function options_setup ( $type ) {
        ...
        $options = apply_filters( 'filters_field_' . $type . '_options', (array) self::$loaded[ $type ]->options(), $type, $core_defaults );
    ...
    public static function fields_setup ( $fields = null, $core_defaults = null, $single = false ) {
        ...
        foreach ( $fields as $f => $field ) {
            $fields[ $f ] = self::field_setup( $field, $core_defaults, filters_var( 'type', $field, 'text' ) );
        }
        ...
        if ( empty( $core_defaults ) ) {
            ...
            if ( null !== $type ) {
                ...
                if ( method_exists( self::$loaded[ $type ], 'options' ) )
                    $options = apply_filters( 'filters_field_' . $type . '_options', (array) self::$loaded[ $type ]->options(), $type );
            }
        }
    ...
    public static function dependencies ( $options, $prefix = '' ) {
        ...
        if ( !empty( $depends_on ) ) {
            $classes[] = 'filters-depends-on';

            foreach ( $depends_on as $depends => $on ) {
                $classes[] = 'filters-depends-on-' . $prefix . self::clean( $depends, true );

                if ( !is_bool( $on ) ) {
                    $on = (array) $on;

                    foreach ( $on as $o ) {
                        $classes[] = 'filters-depends-on-' . $prefix . self::clean( $depends, true ) . '-' . self::clean( $o, true );
                    }
                }
            }
        }
        ...
        if ( !empty( $excludes_on ) ) {
            $classes[] = 'filters-excludes-on';
            foreach ( $excludes_on as $excludes => $on ) {
                $classes[] = 'filters-excludes-on-' . $prefix . self::clean( $excludes, true );

                $on = (array) $on;

                foreach ( $on as $o ) {
                    $classes[] = 'filters-excludes-on-' . $prefix . self::clean( $excludes, true ) . '-' . self::clean( $o, true );
                }
            }
        }
    ...
    public static function display ( $type, $value = null, $name = null, $options = null, $pod = null, $id = null, $traverse = null ) {
        ...
        $tableless_field_types = apply_filters( 'filters_tableless_field_types', array( 'pick', 'file', 'avatar' ) );
    ...
    public static function regex ( $type, $options ) {
        ...
        $regex = apply_filters( 'filters_field_' . $type . '_regex', $regex, $options, $type );
    ...
    public static function prepare ( $type, $options ) {
        ...
        $prepare = apply_filters( 'filters_field_' . $type . '_prepare', $prepare, $options, $type );
    ...
    public static function validate ( $type, &$value, $name = null, $options = null, $fields = null, $pod = null, $id = null, $params = null ) {
        ...
        $validate = apply_filters( 'filters_field_' . $type . '_validate', $validate, $value, $name, $options, $fields, $pod, $id, $type, $params );
    ...
    public static function default_value ( $value, $type = 'text', $name = null, $options = null, $pod = null, $id = null ) {
        $default_value = filters_var_raw( 'default_value', $options, $value, null, true );
        $default = filters_var_raw( 'default', $options, $default_value, null, true );

        $default_value = str_replace( array( '{@', '}' ), '', trim( $default ) );

        if ( $default != $default_value )
            $default = filters_evaluate_tags( $default );

        $default = filters_var_raw( filters_var_raw( 'default_value_parameter', $options ), 'request', $default, null, true );

        if ( $default != $value )
            $value = $default;

        if ( is_array( $value ) )
            $value = filters_serial_comma( $value );

        return apply_filters( 'filters_form_field_default_value', $value, $default, $type, $options, $pod, $id );
    }
    ...
    public static function field_loader ( $field_type ) {
        ...
        if ( class_exists( $class_name ) )
            ...
            $class = new FiltersField();
            $class_name = 'FiltersField';
        }
