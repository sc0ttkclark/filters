 ...
 */
class FiltersField_Color extends FiltersField {
    ...
    public function input ( $name, $value = null, $options = null, $pod = null, $id = null ) {
        ...
        if ( version_compare( $wp_version, '3.5-alpha', '<' ) )
            filters_view( FILTERS_DIR . 'ui/fields/farbtastic.php', compact( array_keys( get_defined_vars() ) ) );
        // WP Color Picker for 3.5+
        else
            filters_view( FILTERS_DIR . 'ui/fields/color.php', compact( array_keys( get_defined_vars() ) ) );
    }
        ...
        if ( is_array( $check ) )
            ...
            if ( 0 < strlen( $value ) && strlen( $check ) < 1 ) {
                if ( 1 == filters_var( 'required', $options ) )
                    ...
                    $errors[] = __( 'Invalid value provided for this field.', 'filters' );
                }
            }
            elseif ( 3 != strlen( $color ) && 6 != strlen( $color ) && 1 != empty( $color ) )
                $errors[] = __( 'Invalid Hex Color value provided for this field.', 'filters' );
        }
